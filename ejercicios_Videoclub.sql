...
WHERE
	...
	WHERE
		DIRECTORS.DIRECTOR_ID = MOVIES.DIRECTOR_ID
		AND DIRECTORS.DIRECTOR_DEAD_DATE < MOVIE_LAUNCH_DATE);

--35
SELECT
	D.DIRECTOR_NAME AS DIRECTOR,
	MAX(MG.GENRE_NAME) AS GENERO_MAS_REPETIDO,
	NumPelis AS CANTIDAD_DE_PELICULAS_DEL_GENERO
FROM
	DIRECTORS D
JOIN (
	SELECT
...
SELECT
	S.STUDIO_NAME AS ESTUDIO,
	MAX(MN.NATIONALITY_NAME) AS NACIONALIDAD_MAS_REPETIDA
FROM
	STUDIOS S
JOIN (
	SELECT
	...
	S.STUDIO_ID = MN.STUDIO_ID
GROUP BY S.STUDIO_NAME

--37
SELECT
	MIEMBROS.MEMBER_NAME AS MIEMBRO,
	PELICULAS_MIEMBROS.MOVIE_NAME AS PELICULA,
	MIEMBROS.FECHA_PRIMERA_PELICULA AS FECHA_ALQUILER,
	MIEMBROS.PHONE
FROM
	(
	SELECT
		ME.MEMBER_NAME,
		ME.MEMBER_ID,
		ME.MEMBER_PHONE AS PHONE,
		MR.FECHA_PRIMERA_PELICULA
	FROM
		MEMBERS ME
	JOIN (
		SELECT
			MEMBER_ID,
			MIN(MEMBER_RENTAL_DATE) AS FECHA_PRIMERA_PELICULA
		FROM
...
JOIN (
	SELECT
		M.MOVIE_NAME,
		M.MOVIE_ID AS MOVIE_ID,
		MR.MEMBER_RENTAL_DATE AS MEMBER_RENTAL_DATE,
		MR.MEMBER_ID AS MEMBER_ID
	FROM
		MOVIES M
	JOIN MEMBERS_MOVIE_RENTAL MR ON
		M.MOVIE_ID = MR.MOVIE_ID	
) PELICULAS_MIEMBROS ON
	MIEMBROS.MEMBER_ID = PELICULAS_MIEMBROS.MEMBER_ID
JOIN NATIONALITIES N ON
		N.NATIONALITY_ID = RIGHT (MIEMBROS.PHONE,1)
WHERE
	PELICULAS_MIEMBROS.MEMBER_RENTAL_DATE = MIEMBROS.FECHA_PRIMERA_PELICULA